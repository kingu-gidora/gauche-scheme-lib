(define-module game.astar
  (use game.grid)
  (export-all))
(select-module game.astar)
    ...
    (let-keywords options ((neighbours (lambda (n) (get-neighbours-func n grid #f)))
		  ...
		  (letrec ((nodes (make-grid (grid 'width) (grid 'height) (make-vector 4 0)))
			   ...
			   (unsolvable? (lambda (n) 
					  (and (null? openL) 
					       (not (equal? n from)))))
			   (in-open? (lambda (n) (member n openL)))
			   ...
			   (add-to-closeL! (lambda (n)
					     (set! openL (remove (lambda (zz) (equal? zz n)) openL))
					     (push! closeL n)))
			   ...
			   ;; 	       'no-path-exists)
			   ;; 	      (else
			   ;; 	       ;(format #t "searching neighboors for: ~a\n" cn)
			   ;; 	       (format #t "N~a : ~a\n" cn (neighbours cn))
